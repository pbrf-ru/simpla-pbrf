...
<div class="message message_error">
	<span>Вы не имеете права на просмотр этого раздела</span>
...
<div id="order_details">
	...
	<ul class="order_details">
		...
		<li>
			<label class=property>Трекинг-номер отправления</label>
			<div class="edit_order_detail" style='display:none;'>
				<input name="delivery_track" class="simpla_inp" type="text" value="{$order->delivery_track|escape}" />
			</div>
			<div class="view_order_detail">
	        {if $order->delivery_track != ''}{$order->delivery_track|escape}{else}Нет данных по трекномеру{/if}
			</div>
		</li>
		<li>
			<label class=property>Состояние отправления</label>
			<div class="edit_order_detail" style='display:none;'>
				<input name="delivery_state" class="simpla_inp" type="text" value="{$order->delivery_state|escape}" />
			</div>
			<div class="view_order_detail">
			{if $order->delivery_state != ''}{$order->delivery_state|escape}{else}Нет данных по состоянию отправления{/if}
			</div>
		</li>
	</ul>
	</div>
	...
	<div class='layer'>
	<h2>Пользователь <a href='#' class="edit_user"><img src='design/images/pencil.png' alt='Редактировать' title='Редактировать'></a> {if $user1}<a href="#" class='delete_user'><img src='design/images/delete.png' alt='Удалить' title='Удалить'></a>{/if}</h2>
		<div class='view_user'>
		{if !$user1}
			Не зарегистрирован
		{else}
			<a href='index.php?module=UserAdmin&id={$user1->id}' target=_blank>{$user1->name|escape}</a> ({$user1->email|escape})
		{/if}
		</div>
...
<div id="purchases">
	...
	<div class="block delivery">
		<h2>Доставка</h2>
				...
				<!--	ИНДИВИДУАЛЬНАЯ ВСТАВКА 8,9 -->
				{if $order->delivery_id == 4 || $order->delivery_id == 9}
					<a href='javascript:void(0);'>
						<img id="print_blank" src='design/images/printer.gif' alt='Печать бланка' title='Печать бланка' border="0" rel="{$order->id}">
					</a>
					<div class="opaco hidden"></div>
					<!--модальное окно для редактирования данных-->
					<div id="modal_window" class="hidden">
						<div class="close">X</div>
						<fieldset>
							<legend>Данные для печати</legend>
							<div class="row">
								<label for="surname">Фамилия</label>
								<input id="surname" value="{$order->fam}">
								<label for="name_client">Имя</label>
								<input id="name_client" value="{$order->imya}">
								<label for="patronomic">Отчество</label>
								<input id="patronomic" value="{$order->otch}">
							</div>
							<div class="row">
								<label for="street">Улица</label>
								<input id="street" value="{$order->ul}">
								<label for="build">Дом</label>
								<input id="build" value="{$order->dom}">
								<label for="appartment">Квартира</label>
								<input id="appartment" value="{$order->kv}">
							</div>
							<div class="row">
								<label for="region">Область</label>
								<input id="region" value="{$order->obl}">
								<label for="city">Город</label>
								<input id="city" value="{$order->city}">
							</div>
							<div class="row">
								<label for="country">Страна</label>
								<input id="country" value="{$order->country}">
								<label for="zip">Индекс</label>
								<input id="zip" value="{$order->zip}">
							</div>
							<div class="row">
								<fieldset class="half">
									<legend>Данные для бланка "Ярлык Ф.7"</legend>
									<div class="row">
										<label for="declared_value" class="big">Сумма объявленной ценности</label>
										<input id="declared_value" value="{$order->total_price}">
									</div>
									<div class="row">
										<label for="cod_amount" class="big">Сумма наложенного платежа</label>
										<input id="cod_amount" value="{$order->total_price}">
									</div>
									<div class="row bottom">
										<a id="pdf_f7" href="javascript:void(0);" rel="false">
											<span>Получить ссылку для скачивания</span>
											<!--ссылка для скачивания появится после нажатия на картинку-->
											<img src='design/images/pdf.png' alt='Ф.7' title='Сохранить бланк ф.7' width="16">	
										</a>
									</div>
								</fieldset>
								<fieldset class="half last">
									<legend>Данные для бланка "Почтовый перевод Ф.112"</legend>
									<div class="row">
										<label for="sum_num">Перевод на сумму</label>
										<input id="sum_num" value="{$order->total_price}">
									</div>
									<div class="row">&nbsp;</div>
									<div class="row bottom">
										<a id="pdf_f112" href="javascript:void(0);" rel="false">
											<span>Получить ссылку для скачивания</span>
											<!--ссылка для скачивания появится после нажатия на картинку-->
											<img src='design/images/pdf.png' alt='Ф.112' title='Сохранить бланк ф.112' width="16">	
										</a>
									</div>
								</fieldset>
							</div>
...
{if $order->avans_price > 0}
	Аванс<b> {$order->avans_price} {$currency->sign}</b><br>
	Остаток<b> {$order->full_price - $order->avans_price} {$currency->sign}</b>
{else}
...
{/if}
	...
	<label for="notify_user">Уведомить покупателя о состоянии заказа</label>
        ...
        <input type="checkbox" value="1" id="notify_about_track" name="notify_about_track">
        <label for="notify_about_track">Уведомить покупателя о присвоении трек-номера</label>
